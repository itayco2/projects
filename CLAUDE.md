...
- Content is project-first and short (2026-09, for AI-engineer roles): hero (availability chip, portrait, name, the claim "I ship production LLM systems and full-stack platforms.", one-sentence lede, two buttons), a `proof` strip with one number per project, the `projects` grid (each card: one visual, a tag naming the skill it proves, name, one line, a link to the code), a short About (`facts`) and contact. The résumé itself is deliberately not repeated. A project is listed only if it proves something the others don't; private repos are never linked.
